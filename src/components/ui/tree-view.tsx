...
const TreeView = React.forwardRef<HTMLDivElement, TreeProps>(
    (
        ...
        const [internalExpandedItems, setInternalExpandedItems] = React.useState<Set<string>>(new Set())
        
        const currentExpandedItems = expandedItems || internalExpandedItems
        const setCurrentExpandedItems = onExpandedChange || setInternalExpandedItems

        React.useEffect(() => {
            if (expandedItems) {
                setInternalExpandedItems(new Set(expandedItems));
            }
        }, [expandedItems]);
        ...
        const expandedItemIds = React.useMemo(() => {
            if (expandedItems && expandedItems.size > 0) {
            ...
            if (expandRootOnly) {
                ...
                function collectRootIds(items: TreeDataItem[] | TreeDataItem) {
                    if (items instanceof Array) {
                        items.forEach(item => {
                            rootIds.push(item.id)
                        })
                    } else {
                        rootIds.push(items.id)
                    }
                }
            ...
            return ids
        }, [data, expandAll, expandRootOnly, initialSelectedItemId, expandedItems])
        ...
        return (
            <div className={cn('overflow-hidden relative p-2', className)}>
                <TreeItem
                    ...
                    setCurrentExpandedItems={setCurrentExpandedItems}
                    {...props}
                />
                <div
...
type TreeItemProps = TreeProps & {
    ...
    setCurrentExpandedItems?: (expandedItems: Set<string>) => void
}
...
const TreeNode = ({
    ...
    const [value, setValue] = React.useState<string[]>(() => {
        return expandedItemIds.includes(item.id) ? [item.id] : []
    })
        ...
        <AccordionPrimitive.Root
            ...
            onValueChange={(s) => {
                ...
                if (setCurrentExpandedItems) {
                    ...
                    setCurrentExpandedItems(newExpandedItems);
                }
            }}
        >
            <AccordionPrimitive.Item value={item.id}>
                <AccordionTrigger
                    className={cn(
                        treeVariants(),
                        selectedItemId === item.id && "bg-blue-50 border-blue-200 shadow-sm",
                ...
                >
                    ...
                    <span 
                        className={cn(
                            "text-sm truncate cursor-pointer transition-colors duration-200",
        ...
        return (
            <div
                ...
                className={cn(
                    ...
                    treeVariants(),
                    className,
                    selectedItemId === item.id && "bg-blue-50 border border-blue-200 shadow-sm",
                    isDragOver && dragOverVariants(),
            ...
            >
                ...
                />
                <span className={cn(
                    "flex-grow text-sm truncate transition-colors duration-200",
                    selectedItemId === item.id 
                        ? "text-blue-700 font-semibold" 
                        : ""
                )}>
