...
export default function EmailSendPage() {
  ...
  const emailMemberSearchInitial = useMemo(() => {
    ...
    return { initialSelectedIds, initialById }
  }, [recipients])
  ...
  const applyRecipientsFromModal = (payload: MemberSearchConfirmPayload) => {
        ...
        <div className="fixed inset-0 z-50 bg-black/40 p-6">
          <div className="mx-auto max-h-[90vh] max-w-5xl overflow-y-auto rounded-2xl bg-white p-6">
            ...
            <MemberSearchPanel
              mode="email"
              showHeader={false}
              initialSelectedIds={emailMemberSearchInitial.initialSelectedIds}
              initialById={emailMemberSearchInitial.initialById}
              manualEmailCount={manualEmails.length}
              onExtraClear={() => setManualEmails([])}
              topSlot={
              ...
              onConfirm={applyRecipientsFromModal}
              onCancel={() => {
                setManualEmails(recipients.filter((x) => x.key.startsWith('e:')).map((x) => x.email))
                setManualEmailInput('')
                setContactModalOpen(false)
