    ...
    <div className="space-y-2 relative">
      <div className="space-y-2 rounded-lg border border-gray-200 bg-white p-4">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          <div className="flex items-center gap-4">
            <label className="w-[86px] text-right whitespace-nowrap text-sm font-medium text-gray-700">시작 등록일</label>
            <Input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} />
            <label className="w-[86px] text-right whitespace-nowrap text-sm font-medium text-gray-700">종료 등록</label>
            <Input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} />
          </div>

          <div className="flex items-center gap-4">
            <label className="w-[65px] text-right whitespace-nowrap text-sm font-medium text-gray-700">카테고리</label>
            <SysCodeSelect
              ...
              allOptionLabel="전체"
            />
            <label className="w-[68px] text-right whitespace-nowrap text-sm font-medium text-gray-700">공개 범위</label>
          ...
          <div className="flex items-center gap-4">
            <label className="w-[48px] text-right whitespace-nowrap text-sm font-medium text-gray-700">상태</label>
          ...
          <div className="col-span-1 flex items-center gap-4 md:col-span-2 lg:col-span-2">
            <label className="w-[48px] text-right whitespace-nowrap text-sm font-medium text-gray-700">검색</label>
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200 bg-white">
        {loading ? (
          ...
          <div className="p-12 text-center text-gray-500">
            {filterBarActive ? '검색 결과가 없습니다.' : '등록된 세미나가 없습니다.'}
          </div>
        ) : (
                  ...
                  return (
                    <tr key={video.id} className="hover:bg-gray-50">
                      ...
                      <td className={cn(VIDEO_LIST_COL_TITLE, 'align-top break-words text-left')}>
                        <Link
                          href={`/admin/seminar/edit?id=${video.id}`}
